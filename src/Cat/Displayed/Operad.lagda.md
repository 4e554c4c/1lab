...
{-
    comp-homl
      : ∀ {z} (mhom : List ΣHoml) → Homl (fst ⊙ snd <$> mhom) z
      → Homl (concat (fst <$> mhom)) z

    idl
      : ∀ {xs y} {h : Homl xs y} →
      PathP (λ i → Homl {! !} y) (comp-homl {! !} {! h !}) h

    idr
      : ∀ {xs y} {h : Homl xs y} →
      PathP (λ i → Homl {! !} y) (comp-homl [ xs ] [ y ] y (λ { fzero → h
                                                              ; (fin (suc n) ⦃ x ⦄) → {! x !} }) (id y)) h
-}
