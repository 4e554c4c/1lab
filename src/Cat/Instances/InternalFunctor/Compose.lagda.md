...
```agda
  (α ◂i H) .mapi x = α .mapi (H .Fi₀ x)
  (α ◂i H) .comi x y f = α .comi _ _ _
  (α ◂i H) .mapi-nat x σ = ℂ.begini
    α .mapi (H .Fi₀ x) [ σ ] ℂ.≡i⟨ α .mapi-nat _ σ ⟩
    α .mapi (H .Fi₀ x ∘ σ)   ℂ.≡i⟨ ap (α .mapi) (H .Fi₀-nat x σ) ⟩
    α .mapi (H .Fi₀ (x ∘ σ)) ∎

  (H ▸i α) .mapi x = H .Fi₁ (α .mapi x)
  (H ▸i α) .comi x y f =
    sym (H .Fi-∘ _ _) ∙ ap (H .Fi₁) (α .comi _ _ _) ∙ H .Fi-∘ _ _
  (H ▸i α) .mapi-nat x σ = ℂ.casti $
    H .Fi₁-nat _ σ ℂ.∙i λ i → H .Fi₁ (α .mapi-nat x σ i)

  _◆i_ {F} {G} {H} {K} α β .mapi x = G .Fi₁ (β .mapi x) ℂ.∘i α .mapi (H .Fi₀ x)
  _◆i_ {F} {G} {H} {K} α β .comi x y f =
    (G .Fi₁ (β .mapi _) ℂ.∘i α .mapi _) ℂ.∘i F .Fi₁ (H .Fi₁ f)   ≡⟨ ℂ.pullri (α .comi _ _ _) ⟩
    G .Fi₁ (β .mapi _) ℂ.∘i (G .Fi₁ (H .Fi₁ f) ℂ.∘i α .mapi _)   ≡⟨ ℂ.pullli (sym (G .Fi-∘ _ _) ∙ ap (G .Fi₁) (β .comi _ _ _)) ⟩
    G .Fi₁ (K .Fi₁ f 𝔹.∘i β .mapi _) ℂ.∘i α .mapi _              ≡⟨ ℂ.pushli (G .Fi-∘ _ _) ⟩
    G .Fi₁ (K .Fi₁ f) ℂ.∘i (G .Fi₁ (β .mapi _) ℂ.∘i α .mapi _)   ∎
  _◆i_ {F} {G} {H} {K} α β .mapi-nat x σ = ℂ.begini
    (G .Fi₁ (β .mapi x) ℂ.∘i α .mapi (H .Fi₀ x)) [ σ ]       ℂ.≡i⟨ ℂ.∘i-nat _ _ _ ⟩
    G .Fi₁ (β .mapi x) [ σ ] ℂ.∘i α .mapi (H .Fi₀ x) [ σ ]   ℂ.≡i⟨ (λ i → G .Fi₁-nat (β .mapi x) σ i ℂ.∘i α .mapi-nat (H .Fi₀ x) σ i) ⟩
    G .Fi₁ (β .mapi x [ σ ]) ℂ.∘i α .mapi (H .Fi₀ x ∘ σ)     ℂ.≡i⟨ (λ i → G .Fi₁ (β .mapi-nat x σ i) ℂ.∘i α .mapi (H .Fi₀-nat x σ i)) ⟩
    G .Fi₁ (β .mapi (x ∘ σ)) ℂ.∘i α .mapi (H .Fi₀ (x ∘ σ))   ∎
```
</details>
...
```agda
  ...
  Fi∘-functor .F-∘ {F , G} {H , J} {K , L} (α , β) (γ , τ) = Internal-nat-path λ x →
    K .Fi₁ (β .mapi _ 𝔹.∘i τ .mapi _) ℂ.∘i α .mapi _ ℂ.∘i γ .mapi _            ≡⟨ ℂ.pushli (K .Fi-∘ _ _) ⟩
    K .Fi₁ (β .mapi _) ℂ.∘i K .Fi₁ (τ .mapi _) ℂ.∘i α .mapi _ ℂ.∘i γ .mapi _   ≡⟨ ℂ.extend-inneri (sym (α .comi _ _ _)) ⟩
    K .Fi₁ (β .mapi _) ℂ.∘i α .mapi _ ℂ.∘i H .Fi₁ (τ .mapi _) ℂ.∘i γ .mapi _   ≡⟨ ℂ.associ _ _ _ ⟩
    (K .Fi₁ (β .mapi x) ℂ.∘i α .mapi _) ℂ.∘i H .Fi₁ (τ .mapi _) ℂ.∘i γ .mapi _ ∎
```
</details>
