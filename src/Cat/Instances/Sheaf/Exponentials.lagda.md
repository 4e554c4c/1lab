...
```agda
  abstract
    ...
    sep {c = c} {x} {y} loc = ext λ V f z → bshf.separate (pull f (inc c)) λ g hg →
      B ⟪ g ⟫ x .map V (f , z)             ≡˘⟨ x .com _ _ _ $ₚ _ ⟩
      x .map _ (f ∘ g , A ⟪ g ⟫ z)         ≡⟨ ap (x .map _) (Σ-pathp (intror refl) refl) ⟩
      x .map _ ((f ∘ g) ∘ id , A ⟪ g ⟫ z)  ≡⟨ loc (f ∘ g) hg ηₚ _ $ₚ _ ⟩
      y .map _ ((f ∘ g) ∘ id , A ⟪ g ⟫ z)  ≡˘⟨ ap (y .map _) (Σ-pathp (intror refl) refl) ⟩
      y .map _ (f ∘ g , A ⟪ g ⟫ z)         ≡⟨ y .com _ _ _ $ₚ _ ⟩
      B ⟪ g ⟫ y .map V (f , z)             ∎
```
...
```agda
  module _ {U} {c : J ʻ U} (p : Patch psh (J .cover c)) where
    ...
    p' e f .part g hg = p .part (f ∘ g) hg .map _ (id , A ⟪ g ⟫ e)
    p' e f .patch g hg h hh =
      B ⟪ h ⟫ p .part (f ∘ g) _ .map _ (id , A ⟪ g ⟫ e)          ≡˘⟨ p .part (f ∘ g) hg .com _ _ _ $ₚ (id , A ⟪ g ⟫ e) ⟩
      p .part (f ∘ g) _ .map _ (id ∘ h , A ⟪ h ⟫ (A ⟪ g ⟫ e))    ≡⟨ ap (λ it → p .part (f ∘ g) hg .map _ (it , A ⟪ h ⟫ (A ⟪ g ⟫ e))) id-comm-sym ⟩
      p .part (f ∘ g) _ .map _ (h ∘ id , A ⟪ h ⟫ (A ⟪ g ⟫ e))    ≡⟨ p .patch (f ∘ g) hg h (subst (_∈ J .cover c) (assoc _ _ _) hh) ηₚ _ $ₚ (id , _) ⟩
      p .part ((f ∘ g) ∘ h) _ .map _ (id , A ⟪ h ⟫ (A ⟪ g ⟫ e))  ≡⟨ app p (sym (assoc f g h)) ηₚ _ $ₚ _ ⟩
      p .part (f ∘ g ∘ h) _ .map _ (id , A ⟪ h ⟫ (A ⟪ g ⟫ e))    ≡⟨ ap (λ e → p .part (f ∘ g ∘ h) hh .map _ (id , e)) (sym (A .F-∘ _ _ · _)) ⟩
      p .part (f ∘ g ∘ h) _ .map _ (id , A ⟪ g ∘ h ⟫ e)          ∎
```
...
```agda
    ...
    s' .whole .map x (f , e) = it .whole module s' where
      it : Section B (p' e f)
      it = bshf.split (pull f (inc c)) (p' e f)

    s' .whole .com x y f = ext λ g e → bshf.separate (pull (g ∘ f) (inc c)) λ h hh →
      let clo = subst (_∈ J .cover c) (sym (assoc _ _ _)) hh in
      B ⟪ h ⟫ (s'.it y (g ∘ f) (A ⟪ f ⟫ e) .whole)            ≡⟨ s'.it y (g ∘ f) (A ⟪ f ⟫ e) .glues _ hh ⟩
      p .part ((g ∘ f) ∘ h) _ .map _ (id , A ⟪ h ⟫ (A ⟪ f ⟫ e)) ≡˘⟨ (λ i → p .part (assoc g f h i) (coe1→i (λ i → assoc g f h i ∈ J .cover c) i hh) .map _ (id , A .F-∘ h f i e)) ⟩
      p .part (g ∘ f ∘ h) _ .map _ (id , A ⟪ f ∘ h ⟫ e)         ≡˘⟨ sym (B .F-∘ _ _ · _) ∙ s'.it x g e .glues (f ∘ h) clo ⟩
    ...
    s' .glues f hf = ext λ x g e →
      ...
      p .part ((f ∘ g) ∘ id) _ .map x (id , A ⟪ id ⟫ e)  ≡⟨ ap₂ (λ i1 i2 → p .part i1 i2 .map x (id , A ⟪ id ⟫ e)) (idr (f ∘ g)) prop! ⟩
      p .part (f ∘ g) _ .map x (id , A ⟪ id ⟫ e)         ≡⟨ sym (p .patch f hf g (J .cover c .closed hf g) ηₚ _ $ₚ (id , A ⟪ id ⟫ e)) ⟩
      p .part f hf .map x (g ∘ id , A ⟪ id ⟫ e)          ≡⟨ ap (p .part f hf .map x) (Σ-pathp (idr g) (A .F-id · _)) ⟩
      p .part f hf .map x (g , e)                        ∎
